...
Recent works show that diagonal posterior covariances ($$\Sigma_x$$) cause the Hessian of $$\log p_\theta(x\mid z)$$ to be *approximately* diagonal, causing column orthgonality in $$J_z$$, which can be made more precise [(Opper & Achambeau, 2009)](http://www0.cs.ucl.ac.uk/staff/c.archambeau/publ/neco_mo09_web.pdf):
