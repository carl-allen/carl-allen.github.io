...
**Disentanglement** is an intriguing phenomenon in observed in generative latent variable models, particularly _Variational Autoencoders_ (VAEs). Disentanglement is not a rigorously defined term but refers to when semantically meaningful factors of the data map to distinct dimensions in the latent space. This allows, for example, images to be generated that vary only in a particular feature (e.g. smile, orientation) by changing a *single latent dimension*.
...
<p align="center">
        <img src="/assets/disentanglement/faces.png" 
                ...
                height="130" 
                style="display: block; margin: 0 auto"  />
        From <a href="[url](https://openreview.net/forum?id=Sy2fzU9gl)">Higgins et al. (ICLR, 2017)</a>: each row of images is generated by varying a single dimension of the latent variable or, equivalently, moving in an "axis-aligned" direction in latent space. Each row predominantly varies in a single semantic factor: rotation, smile.
</p>
