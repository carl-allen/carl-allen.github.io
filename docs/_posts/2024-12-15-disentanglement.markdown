...
|In this post, we summarise [Unpicking Data at the Seams: VAEs, Disentanglement and Independent Components (Allen, 2024)][paper], which explains **disentangelement in generative latent variable models**. While disentanglement is often associated with particular model families (e.g. VAE, $\beta$-VAE) whose popularity ebb and flow, we see that the phenomenon itself **relates to the latent structure of the data** and is more fundamental than any particular model class that exposes it.|
|---|

**Disentanglement** is an intriguing phenomenon observed in generative latent variable models, particularly [_Variational Autoencoders_ (VAEs)][VAE]. Disentanglement is not a rigorously defined term but refers to when semantically meaningful factors of the data map to distinct dimensions in the latent space. This allows, for example, images to be generated that vary in a specific feature (e.g. smile, orientation) by changing a *single latent dimension*.
...
<p align="center">
        ...
        <img src="/assets/disentanglement/faces.png" 
                ...
                height="130" 
                style="display: block; margin: 0 auto"  />
        From <a href="[url](https://openreview.net/forum?id=Sy2fzU9gl)">Higgins et al. (ICLR, 2017)</a>: each row of images, which mainly vary by a single semantic factor (rotation, smile), are generated by varying a single dimension of the latent variable or, equivalently, moving in an "axis-aligned" direction in latent space.
</p>

**Motivation**: Disentanglement is particularly intriguing because VAEs were not deliberately designed to achieve it and occurs in settings (i.e. with spherically symmetric priors) where it has been considered impossible,[^locatello][^khemakem] thus its understanding may provide new insight into how and what VAEs learn. More generally, the ability to separate independent aspects of the data could be useful in many machine learning domains and, by teasing apart their generative factors, offer fundamental insights into the underlying data itself. 

Several recent works[^rolinek][^kumarpoole] suggest that disentanglement in VAEs may result from commonly used *diagonal posterior covariance matrices* promoting *column-orthgonality in the decoder's Jacobian matrix*. Building on this idea, [Allen (2024)][paper] (A) clarifies this connection, and (B) explains how it leads to disentanglement, showing that **disentanglement amounts to factorising the data distribution into *statistically independent components***. 
$$
\begin{equation}
...
A VAE assumes a latent generative model for data $$x\in\mathcal{X} \subseteq\mathbb{R}^n$$: independent latent variables $$z\in\mathcal{Z} =\mathbb{R}^m$$, $$z\sim p(z) = \prod_ip(z_i)$$, undergo a stochasitic mapping from latent to data space $$x\sim p(x|z)$$. The latent prior is commonly assumed an isotropic Gaussian $$p_\theta(z_i) = \mathcal{N}(z_i,0,1)$$. We also assume $$p(x|z)$$ is Gaussian.
