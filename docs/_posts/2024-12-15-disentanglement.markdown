...
**Motivation**: Disentanglement is particularly intriguing because VAEs were not deliberately designed to achieve it thus its understanding may provide new insights into how and what VAEs learn. Furthermore, it occurs in settings where it has been considered impossible, i.e. where the latent prior is spherically symmetric.[^locatello][^khemakem] More generally, the ability to separate independent aspects of the data is relevant in many machine learning tasks and domains; and, by teasing apart their generative factors, may offer fundamental insights into the underlying data itself. 
...
A VAE assumes a latent generative model for data $$x\in\mathcal{X} \subseteq\mathbb{R}^n$$: independent latent variables $$z\in\mathcal{Z} =\mathbb{R}^m$$, $$z\sim p(z) = \prod_ip(z_i)$$, undergo a stochasitic mapping from latent to data space $$x\sim p(x\mid z)$$. The latent prior is commonly assumed an isotropic Gaussian $$p_\theta(z_i) = \mathcal{N}(z_i,0,1)$$. We also assume $$p(x\mid z)$$ is Gaussian.
