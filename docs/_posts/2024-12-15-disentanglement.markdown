...
**Approach**: Recent works[^rolinek][^kumarpoole] suggest that disentanglement in VAEs may result from commonly used *diagonal posterior covariance matrices* promoting *column-orthgonality in the decoder's Jacobian matrix*. [Allen (2024)][paper] builds on this by: (A) clarify the connection between diagonal covariance and column-orthogonality and (B) explain how that leads to disentanglement; ultimately showing that **disentanglement corresponds to factorising the data distribution into *statistically independent components***.
...
As in a VAE (or GAN), we assume that data $$x\in\mathcal{X} \subseteq\mathbb{R}^n$$ are generated by a latent variable model: independently sampled latent variables $$z\in\mathcal{Z} =\mathbb{R}^m$$, $$p(z) = \prod_ip(z_i)$$, are mapped stochastically to data space $$x\sim p(x\mid z)$$. The latent prior is commonly assumed to be isotropically Gaussian $$p_\theta(z_i) = \mathcal{N}(z_i,0,1)$$. We also assume $$p(x\mid z)$$ is <abbr title="A GAN can be thought of a limiting case as Var[x|z] tends to 0.">Gaussian</abbr>.
...
* Since a VAE's objective is maximised if the model distribution matches that of the data, if data are generated under that model -- and the ground truth distribution therefore factorises into statistically independent factors -- then (subject to uniqueness), independent components of the model corresponding to distinct latent variables align with independent components of the data (disentanglement).
