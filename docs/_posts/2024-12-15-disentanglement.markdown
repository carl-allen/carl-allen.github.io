...
**TL;DR**: 
1. Disentanglement provably occurs in the _linear_ case, $$p_\theta(x\mid z) = \mathcal{N}(x; Dz,\sigma^2I),\ D\in\mathbb{R}^{n\times m}$$, corresponding to [probabilistic PCA (PPCA)][PPCA]. Of the set of analytically known solutions, a VAE with diagonal posterior covariance finds only those in which latent dimensions $$z_i$$ map to independent factors of variation in the data distribution, i.e. it _disentangles_ $$p(x)$$.
2. Somewhat surprisingly, the rationale for the linear case (described in the test of the post) extends to non-linear VAEs with diagonal posterior covariances which encourage columns of the decoder's Jacobian to be (approximately) orthogonal, which in turn causes independent latent variables to pass through the decoder and emerge (in $$\mathcal{X}$$) as statistically independent components over sub-manifolds of the full decoder-defined manifold.
   * i.e. diagonal covariances cause the decoder to map independent factors of $p(z)$ to independent components that factorise the push-forward distribution over the manifold.
3. A VAE's objective is maximised if the model distribution matches that of the data. Hence, if the data distribution factorises into independent components, then independent components of the model distribution must fit those of the data. But those of the model (approximately) align with distinct latent variables (from 2), meaning that independent factors of $$p(x)$$ correspond to distinct latent variables of the model, achieving disentanglement.
