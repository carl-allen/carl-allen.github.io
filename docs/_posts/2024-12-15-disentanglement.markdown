...
$$\bullet$$ **Approach**: Recent works suggest that disentanglement arises in VAEs because commonly used diagonal posterior covariance matrices promote column-orthogonality in the decoder's Jacobian.[^rolinek]$$^,$$[^kumarpoole] As empirical support, columns of the decoder Jacobian are found to be closer to orthogonal in VAEs with diagonal covariance than those with full covariance or general auto-encoders;[^rolinek] and directly imposing column-orthogonality is shown to induce disentanglement.[^kumarpoole] 
