...
<p align="center">
    <small>Figure 1. Illustrating disentangelement: <it>(left)</it> Each row of images varies by a single semantic feature: hair style, line thickness, orientation. A feature is considered disentangled if such images can be generated by varying a single latent dimension, i.e. distinct latent variables control distinct features. From a geometric perspective, this equates to moving  in an "axis-aligned" direction, or parallel to a standard basis vector, in latent space. <it>(right)</it> Using disentangelement to change the style of an image in diffusion models.</small>
</p>
<p align="center">
