...
The aim here is to understand how independent dimensions $$z_i\in\mathcal{Z}$$ pass through the decoder. By considering $$x$$ in the "$$U$$-basis" we see that independent $$z_i$$ become independent components $$x^{(U)}_i$$, since:
1. $$\{x^{(U)}_i\}_i$$ each vary only with a distinct $z_i$ and so are *independent*;
2. the push-forward of $$d$$ restricted to $$\mathcal{Z^{(i)}}$$ has density $$p(x^{(U)}_i) = s_i^{-1}p(z_i)$$ over $$\mathcal{M}_D^{(i)}$$;
