...
Although disentanglement is associated with certain model families whose popularity may ebb and flow, e.g. VAE, $\beta$-VAE, GANs, we show that the phenomenon itself **relates to the latent structure of the data** and is more fundamental than any particular model that may expose it.
...
**Motivation**: Disentanglement in VAEs is particularly intriguing because they were not intentially designed to achieve it thus its understanding may provide new insights into how and what those models learn. Furthermore, disentanglement is observed even when the latent prior is spherically symmetric, where it is deemed impossible.[^locatello][^khemakem] More generally, the ability to separate independent aspects of the data is pertinent to many areas of machine learning and, by teasing apart their generative factors, may offer fundamental insights into the underlying data itself. 
...
Ultimately this show how **disentanglement corresponds to factorising the data distribution into *statistically independent components***.

---

## Problem set-up:

As in a VAE (or GAN), we assume that data $$x\in\mathcal{X} \subseteq\mathbb{R}^n$$ are generated by a latent variable model as follows: independent latent variables $$z\in\mathcal{Z} =\mathbb{R}^m$$, $$z\sim p(z) = \prod_ip(z_i)$$, undergo a stochasitic mapping from latent to data space $$x\sim p(x\mid z)$$. The latent prior is commonly assumed an isotropic Gaussian $$p_\theta(z_i) = \mathcal{N}(z_i,0,1)$$. We also assume $$p(x\mid z)$$ is <abbr title="A GAN can be thought of a limiting case as Var[x|z] tends to 0.">Gaussian</abbr>.
