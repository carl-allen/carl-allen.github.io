...
Crucially, the Jacobian of the mapping from $$z$$ to $$x^{(U)}$$ is a diagonal matrix $$S$$ and $$\tfrac{\partial x^{(U)}_i}{\partial z_j} =\{s_i\doteq S_{i,i} \text{ if }i=j; 0 \text{ o/w}\}$$. It follows that independent $$z_i$$ map to independent components $$x^{(U)}_i$$ since:
