...
**TL;DR**: 
* Disentanglement provably occurs in the _linear_ case ($$p_\theta(x\mid z) = \mathcal{N}(x; Dz,\sigma^2I),\ D\in\mathbb{R}^{n\times m})$$. While the full set of solutions are known analytically from [probabilistic PCA (PPCA)][PPCA], a VAE with diagonal posterior covariance finds a specific subset of those solutions in which **latent dimensions $$z_i$$ map to independent factors of variation** in the data distribution (disentanglement).
