...
**Problem set-up**: As in a VAE (or GAN), we assume that data $$x\in\mathcal{X} \subseteq\mathbb{R}^n$$ are generated by a latent variable model: independently sampled latent variables $$z\in\mathcal{Z} =\mathbb{R}^m$$, $$p(z) = \prod_ip(z_i)$$, are mapped stochastically to data space <abbr title="A GAN can be thought of a limiting case as Var$$[x|z] \to 0$$.">$$x\sim p(x\mid z)$$</abbr>. The latent prior is commonly assumed to be isotropically Gaussian $$p_\theta(z_i) = \mathcal{N}(z_i,0,1)$$. We also assume $$p(x\mid z)$$ is Gaussian.
